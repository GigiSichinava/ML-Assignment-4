# FER2013 მონაცემების ჩატვირთვა
# train.csv: ორი სვეტი, emotion (0-დან 6-მდე) და pixels (2304 რიცხვი space-ით)
# test.csv: მხოლოდ pixels, ეს Kaggle-ის submission-ისთვისაა
...
IMG_SIZE = 48
# FER2013-ის mean და std normalize-ისთვის
FER_MEAN = 0.5077
FER_STD = 0.2550


def _pixels_to_array(pixel_strings):
    # space-ით გაყოფილ რიცხვებიან სტრიქონებს ვაქცევ (N, 48, 48) მასივად
    arr = np.array(
    ...
    def _transform(self, img):
        ...
        if self.augment:
            ...
            if np.random.rand() < 0.5:
                img = np.fliplr(img).copy()        # ჰორიზონტალური ასახვა
            # მცირე random shift, კიდეების padding-ით
            pad = 4
...
def get_dataloaders(cfg):
    ...
    # class weights: Disgust კლასი ცოტაა, ამიტომ მას მეტ წონას ვაძლევ
...
def prepare_data(data_dir="data"):
    ...
    # tar.gz თუ არსებობს, ჯერ ამოვშალო
    for tg in glob.glob(os.path.join(data_dir, "*.tar.gz")):
        with tarfile.open(tg) as t:
            t.extractall(data_dir)

    # fer2013.csv-დან (emotion, pixels, Usage) ავაწყობ train/test-ს, თუ ცალკე არ მოგვცეს
